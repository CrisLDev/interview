...
import { getAllItemsByUserLoged } from "../services/item.service"
import Card from "./components/MiniComponents/Card/Card"

const myitems = () => {
  const { isLoading, error, data } = useQuery("allMyItems", getAllItemsByUserLoged)
  setAuthToken();
  return (
          ...
          {!isLoading &&
            ...
            data.data.map((item: any) => (
              <div className="relative">
                <Card item={item} key={item.id} own={true} />
              </div>
            ))}
        </dl>
      </div>
